    ...
    for (let components of routes) {
        for (let group of components.groups) {
            for (let route of group.routes) {
                const {app, file, path} = parser(route, components, group, 'wrk');

                const threads = 1
                const connections = 3
                const duration = 10

                console.info(`Running HTTP-Tests for ${app.toUpperCase()} route: ${route.url}...`)
                ...
                try {
                    const cmd = await exec(`wrk -t${threads} -c${connections} -d${duration}s ${route.url}`);
                    stdout = cmd.stdout;
                    stderr = cmd.stderr;
